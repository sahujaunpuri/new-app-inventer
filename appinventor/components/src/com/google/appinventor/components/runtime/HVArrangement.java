...
/**
 * A container for components that arranges them linearly, either horizontally
 * or vertically.
 *
 * @author sharon@google.com (Sharon Perl)
 * @author kkashi01@gmail.com (Hossein Amerkashi) (added Image and
 *         BackgroundColors)
 */

@SimpleObject
public class HVArrangement extends AndroidViewComponent implements Component,
		ComponentContainer {
	...
	public void setChildWidth(final AndroidViewComponent component, int width,
		...
		if (cWidth == 0 && trycount < 2) { // We're not really ready yet...
			...
			androidUIHandler.postDelayed(new Runnable() {
				...
				public void run() {
					System.err
							.println("(HVArrangement)Width not stable yet... trying again");
					setChildWidth(component, fWidth, trycount + 1);
				}
			}, 100); // Try again in 1/10 of a second
		}
		if (width <= LENGTH_PERCENT_TAG) {
			System.err.println("HVArrangement.setChildWidth(): width = "
					+ width + " parent Width = " + cWidth + " child = "
					+ component);
		...
		if (orientation == ComponentConstants.LAYOUT_ORIENTATION_HORIZONTAL) {
			ViewUtil.setChildWidthForHorizontalLayout(component.getView(),
					width);
		} else {
	...
	public void setChildHeight(final AndroidViewComponent component, int height) {
		...
		if (cHeight == 0) { // Not ready yet...
			...
			androidUIHandler.postDelayed(new Runnable() {
				...
				public void run() {
					System.err
							.println("(HVArrangement)Height not stable yet... trying again");
		...
		if (orientation == ComponentConstants.LAYOUT_ORIENTATION_HORIZONTAL) {
			ViewUtil.setChildHeightForHorizontalLayout(component.getView(),
					height);
		} else {
			ViewUtil.setChildHeightForVerticalLayout(component.getView(),
					height);
		}
	}
	 ...
	 */
	@SimpleProperty(category = PropertyCategory.APPEARANCE, description = "A number that encodes how contents of the arrangement are aligned "
	...
	public void AlignHorizontal(int alignment) {
		try {
			...
			container.$form().dispatchErrorOccurredEvent(this,
					"HorizontalAlignment",
					ErrorMessages.ERROR_BAD_VALUE_FOR_HORIZONTAL_ALIGNMENT,
					alignment);
		}
	}
	 ...
	 * aligned at the bottom. Alignment has no effect if the arrangement's
	 * height is automatic.
	 */
	...
	public void AlignVertical(int alignment) {
		try {
			...
			container.$form().dispatchErrorOccurredEvent(this,
					"VerticalAlignment",
					ErrorMessages.ERROR_BAD_VALUE_FOR_VERTICAL_ALIGNMENT,
					alignment);
		}
	}
	...
	public void Image(String path) {
		// If it's the same as on the prior call and the prior load was
		// successful,
		// do nothing.
		...
		if (imagePath.length() > 0) {
			try {
				backgroundImageDrawable = MediaUtil.getBitmapDrawable(
						container.$form(), imagePath);
			} catch (IOException ioe) {
				// Fall through with a value of null for
				// backgroundImageDrawable.
			}
		}

		// Update the appearance based on the new value of
		// backgroundImageDrawable.
		updateAppearance();
	}

	// Update appearance based on values of backgroundImageDrawable,
	// backgroundColor and shape.
